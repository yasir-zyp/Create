...
public class CategoryController {
    ...
    public ServerResponseEntity<List<CategoryVO>> categoryList(@RequestParam(value = "parentId", defaultValue = "0") Long parentId) {
        List<CategoryVO> categories = categoryService.categoryList(Constant.PLATFORM_SHOP_ID,parentId);
        return ServerResponseEntity.success(categories);
    }

    @GetMapping("/shop_category_list")
    @ApiOperation(value = "平台的全部分类列表接口", notes = "平台分类列表接口")
